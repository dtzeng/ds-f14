...
public class ChildManager implements Runnable {
	...
	try {
	    ...
	    ChildToProcess connect = new ChildToProcess("newchild", null,
		    localHost, localPort);
	    ...
	    oos.flush();
	    ChildToProcessResponse response = (ChildToProcessResponse) ois
		    .readObject();
	    ...
	    System.out
		    .println("ChildManager failed to connect to master. Try again.");
	    System.exit(-1);
	} finally {
	    try {
		if (oos != null)
	...
	while (true) {
		...
		new Thread(new ChildManagerServeConnection(clientSocket,
			processes, masterHost, masterPort)).start();
	    } catch (Exception e) {
		e.printStackTrace();
	    }
	}
