...
public class ChildManager implements Runnable {
    ...
    public ChildManager(String masterHost, int masterPort) {
        ...
        try {
            ...
            if(!response.getSuccess()) throw new Exception("Connection to master node failed.");
            else System.out.println("Connection to master node succeeded");
        } catch (Exception e) {
    ...
    public void run() {
        while(true) {
            ...
            try {
                ...
                new Thread(new ChildManagerServeConnection(clientSocket, processes, masterHost, masterPort)).start();
            } catch (Exception e) {
                e.printStackTrace();
            }
        }
