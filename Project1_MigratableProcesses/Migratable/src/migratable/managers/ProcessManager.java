...
public class ProcessManager {
	...
	for (ProcessState process : allProcesses) {
	    System.out.println("Process: " + process.getName() + ", "
		    + "Command: " + process.getCommand() + ", " + "Child: "
		    + process.getChild() + ", " + "Status: "
		    + process.getSuspended());
	}
    }
	...
	Iterator<Map.Entry<String, HostPort>> iterator = children.entrySet()
		.iterator();
	while (iterator.hasNext()) {
	    Map.Entry<String, HostPort> kv = iterator.next();
	    System.out.println("Child: " + kv.getKey() + ", Server: "
	...
	System.out
		.println("To migratable: 'migratable <processname> <destchild>'");
	System.out
		.println("To see currently connection children: 'list children'");
	System.out
		.println("To see currently running processes: 'list processes'");
    ...
    private ProcessToChildResponse sendRequestToChild(ProcessToChild request,
	    HostPort child) {
	Socket socket = null;
	...
	if (cmd.equals("") || splits.length == 0) {
		...
		System.out.println("A process with the name '" + name
	    ...
	    if (child == null) {
		System.out.println("No child with the name '" + splits[1]
			+ "' exists. Try again.");
		return false;
	    }
		...
		processes.put(name, new ProcessState(name, command, "running",
			splits[1]));
		System.out.println("Process '" + name
			+ "' successfully started on " + splits[1]);
	    }
	    return false;
		...
		System.out.println("No process with the name '" + splits[1]
			+ "' exists. Try again.");
		return false;
	    }
	    HostPort child = children.get(processState.getChild());
	    ProcessToChild request = new ProcessToChild("suspend", splits[1],
		    null);
	    ProcessToChildResponse response = sendRequestToChild(request, child);
	    if (response == null || !response.getSuccess()) {
		System.out
			.println("Suspend process failed. Process already finished running.");
	    } else {
		processState.setSuspended("suspended");
		System.out.println("Process '" + splits[1]
	    ...
	    HostPort child = children.get(processState.getChild());
	    ProcessToChild request = new ProcessToChild("resume", splits[1],
		    null);
	    ProcessToChildResponse response = sendRequestToChild(request, child);
	    if (response == null || !response.getSuccess()) {
		System.out
			.println("Resume process failed. Process already finished running.");
	    } else {
	    ...
	    if (response == null || !response.getSuccess()) {
		System.out
			.println("Kill process failed. Process already finished running.");
	    } else {
		System.out.println("Process '" + splits[1]
			+ "' successfully killed");
	    }
	    ...
	    if (childDest == null) {
		System.out.println("No child with the name '" + splits[2]
			+ "' exists. Try again.");
		return false;
	    }

	    ProcessToChild request = new ProcessToChild("migrate", splits[1],
		    null);
	    ProcessToChildResponse response = sendRequestToChild(request,
		    childSource);
	    if (response == null || !response.getSuccess()) {
		System.out
			.println("Migrate process failed. Process already finished running.");
		return false;
	    }
	    MigratableProcess process = response.getProcess();
	    ProcessToChild migrate = new ProcessToChild("run", splits[1],
		    process);
		...
		System.out.println("Process '" + splits[1]
			+ "' successfully migrated to " + splits[2]);
	    }
		...
		ProcessToChild request = new ProcessToChild("shutdown", null,
			null);
