...
public class ChildManagerServeConnection implements Runnable {
    ...
    public ChildManagerServeConnection(Socket socket,
	    Map<String, ProcessThread> processes, String masterHost,
	    int masterPort) {
	this.socket = socket;
	...
	try {
		...
		Thread wrap = new Thread(new ProcessRunner(processThread,
			processes, masterHost, masterPort));
		wrap.start();
		...
		if (processThread == null)
			...
			System.out.println("Suspended process '"
				+ processThread.getProcess().getName() + "'");
		    } else {
		...
		if (processThread == null)
		    ...
		    response = new ProcessToChildResponse(true,
			    processThread.getProcess());
		}
	    }
	    oos.writeObject(response);
	    oos.flush();
