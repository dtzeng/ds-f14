...
public class ChildManagerServeConnection implements Runnable {
  ...
  public ChildManagerServeConnection(Socket socket, Map<String, ProcessThread> processes,
  ...
  public void run() {
    ...
    try {
      ...
      if (ptc.getCommand().equals("shutdown")) {
        ...
        Thread wrap =
            new Thread(new ProcessRunner(processThread, processes, masterHost, masterPort));
        wrap.start();
        ...
        if (processThread == null)
          response = new ProcessToChildResponse(false, null);
        else {
          if (processThread.getThread().isAlive()) {
            ...
            System.out.println("Process '" + processThread.getProcess().getName()
                + "' already suspended");
          }
          response = new ProcessToChildResponse(true, null);
        }
        ...
        if (processThread == null)
          ...
          System.out.println("Stopped process '" + processThread.getProcess().getName()
              + "' and returning to master");
