...
public abstract class Reducer implements Task {
	...
	try {
		...
		// Replace minKV
		Map.Entry<String, String> replacement = readers.get(minIndex)
			.readNextKV();
		if (replacement == null) {
		...
		for (int x = 0; x < kvs.size(); x++) {
		    while (kvs.get(x) != null
			    && compareTo(kvs.get(x).getKey(), minKV.getKey()) == 0) {
			groupMinKey.add(kvs.get(x).getValue());
			Map.Entry<String, String> replace = readers.get(x)
				.readNextKV();
			kvs.set(x, replace);
			if (replace == null) {
			    readers.get(x).close();
		...
		Iterator<Map.Entry<String, String>> kvsIterator = kvs
			.iterator();
