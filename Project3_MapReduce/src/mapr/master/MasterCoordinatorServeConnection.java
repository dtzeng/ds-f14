...
public class MasterCoordinatorServeConnection implements Runnable {
	...
	try {
		...
		synchronized (lock) {
		     ...
		     * || status.contains("RESTARTED")) { iterator.remove(); } }
		     */
		}
		...
		synchronized (lock) {
		    int jobID = startJob("count", user, filename, start, end,
			    output, "");
		    oos.writeInt(jobID);
		}
		oos.flush();
		...
		synchronized (lock) {
		    int jobID = startJob("grep", user, filename, start, end,
			    output, otherArgs);
		    oos.writeInt(jobID);
		}
		oos.flush();
		...
		synchronized (lock) {
		    if (runningTasks.get(workerName) == null
			    || queuedTasks.get(workerName) == null) {
			return;
		    }
			...
			while (true) {
			    TaskInfo map = queuedTasks.get(workerName)
				    .nextMap();
			    if (map == null)
				break;
			    ...
			    if (jobs.get(nextJobID) == null
				    || jobs.get(nextJobID).getStatus()
					    .equals("FAILED")
				    || jobs.get(nextJobID).getStatus()
					    .contains("RESTARTED")) {
				continue;
			    } else if (!sendTaskToWorker(workerName, map)) {
				if (!jobs.get(nextJobID).getStatus()
					.contains("RESTARTED"))
				    jobs.get(nextJobID).setStatus("FAILED");
			    } else {
				runningTasks.get(workerName).addMap(
					map.getTaskID(), map);
		...
		synchronized (lock) {
			...
			while (true) {
			    TaskInfo sort = queuedTasks.get(workerName)
				    .nextSort();
			    if (sort == null)
				break;
				...
				runningTasks.get(workerName).addSort(
					sort.getTaskID(), sort);
				jobs.get(nextJobID).setStatus("RUNNING");
				break;
			    }
		...
		synchronized (lock) {
		    if (jobs.get(jobID) == null
			    || jobs.get(jobID).getStatus().equals("FAILED")) {
			return;
		    } else {
			...
			jobNo = "job"
				+ Integer.toString(jobs.get(jobID).getJobID());
		    }
		}
		...
		if (hpUser != null) {
		    ...
		    try {
			socketUser = new Socket(hpUser.getHost(),
				hpUser.getPort());
			oosUser = new ObjectOutputStream(
		...
		synchronized (lock) {
			...
			while (true) {
			    TaskInfo reduce = queuedTasks.get(workerName)
				    .nextReduce();
			    if (reduce == null)
				break;
				...
				runningTasks.get(workerName).addReduce(
					reduce.getTaskID(), reduce);
				jobs.get(nextJobID).setStatus("RUNNING");
				break;
			    }
			}
