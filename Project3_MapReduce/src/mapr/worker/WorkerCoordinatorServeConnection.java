...
public class WorkerCoordinatorServeConnection implements Runnable {
    ...
    public WorkerCoordinatorServeConnection(Socket socket, String dfsDir,
	...
	try {
		...
		if (task.getTaskType().equals("map")) {
		    runner = new WordCountMapper(task.getInput(),
			    task.getOutput(), task.getRecordStart(),
			    task.getRecordEnd(), recordLength,
			    task.getOtherArgs());
		} else if (task.getTaskType().equals("sort")) {
		    runner = new WordCountSorter(task.getInput(),
			    task.getOutput(), task.getOtherArgs());
		} else if (task.getTaskType().equals("reduce")) {
		    runner = new WordCountReducer(task.getFilenames(),
			    task.getOutput(), task.getOtherArgs());
		}
		...
		if (task.getTaskType().equals("map")) {
		    ...
		    runner = new GrepSorter(task.getInput(), task.getOutput(),
			    task.getOtherArgs());
		} else if (task.getTaskType().equals("reduce")) {
		    runner = new GrepReducer(task.getFilenames(),
			    task.getOutput(), task.getOtherArgs());
		}
	    }
	...
	TaskThreadRunner runner = new TaskThreadRunner(taskThread, task,
		masterHost, masterPort, workerName, tasks);
	...
	try {
		...
		RandomAccessFile file = new RandomAccessFile(dfsDir + "/"
			+ filename, "rws");
		file.write(contents);
		file.close();
