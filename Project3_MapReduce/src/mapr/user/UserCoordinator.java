...
public class UserCoordinator implements Runnable {
    ...
    public UserCoordinator(String name, String host, int port, String dfsRoot, String masterHost, int masterPort, int partitionSize) {
        this.name = name;
        this.host = host;
        this.port = port;
    ...
    public void printHelp() {
        ...
        System.out.println("\tStarts a grep for the given file from records [start, end] with the given match");
        System.out.println("\tResults are put into specified output in user's DFS root.");
    ...
    public boolean parseCommand(String cmd) {
        ...
        if(cmd.equals("") || splits.length == 0) {
            ...
            try {
                ...
                oos.writeUTF("jobs");
                oos.flush();
                ConcurrentHashMap<Integer, JobInfo> jobs = (ConcurrentHashMap<Integer, JobInfo>) ois.readObject();
            ...
            try {
                ...
                oos.writeUTF("files");
                oos.flush();
                ConcurrentHashMap<String, FileInfo> files = (ConcurrentHashMap<String, FileInfo>) ois.readObject();
            ...
            try {
                ...
                System.out.println("<start> and <end> must be integers. Usage: count <input> <start> <end> <output>");
                return false;
            }
            ...
            if(splits.length < 6) {
                System.out.println("Not enough arguments. Usage: grep <input> <start> <end> <output> <match>");
                return false;
            }
            String filename = splits[1];
            int start = 0;
            ...
            try {
                ...
                System.out.println("<start> and <end> must be integers. Usage: grep <input> <start> <end> <output> <match>");
                return false;
            }
            String output = splits[4];
    ...
    public static void main(String args[]) {
        ...
        UserCoordinator coordinator =
                new UserCoordinator(args[1], userHost, Integer.parseInt(userPort), dfsRoot,
                                    masterHost, Integer.parseInt(masterPort), Integer.parseInt(partitionSize));
